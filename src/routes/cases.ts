...
function mapEvidenceForResponse<T extends { type: string }>(
  evidence: T
): Omit<T, 'type'> & { evidenceType: T['type'] } {
  const { type, ...rest } = evidence;
  return { ...rest, evidenceType: type };
}
      ...
      const caseRecord = await prisma.case.create({
        data: {
          ...
          submissionDeadline: body.submissionDeadline
            ...
            : null,
          intakeData: body.intakeData ? toJsonValue(body.intakeData) : undefined,
          status: 'INTAKE',
        },
      });
    ...
    try {
      ...
      res.status(200).json({
        ...
        data: {
          case: {
            ...caseRecord,
            evidence: caseRecord.evidence.map(mapEvidenceForResponse),
          },
        },
      });
    } catch (err) {
    ...
    try {
      ...
      const updated = await prisma.case.update({
        ...
        data: {
          ...
          ...(body.intakeData !== undefined && {
            intakeData: toJsonValue(body.intakeData),
          }),
        },
      });
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      await prisma.case.update({
        where: { id: req.params.caseId },
        data: {
          nextQuestions: toJsonValue(questionsResult),
      ...
      try {
        ...
        const updatedCase = await prisma.case.update({
          ...
          data: {
            ...
            legalArguments: toJsonValue(result.legalArguments),
            recommendedActions: toJsonValue(result.recommendedActions),
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        withSummaries.map((pf) =>
          prisma.evidence.create({
            data: {
              ...
              summary: pf.summary,
              metadata: toJsonValue(pf.metadata),
      ...
      const evidence = created.map(mapEvidenceForResponse);
