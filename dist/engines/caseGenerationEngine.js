...
exports.generateCaseDocument = generateCaseDocument;
const aiService_1 = require("../services/aiService");
const logger_1 = require("../lib/logger");
function buildCaseSystemPrompt() {
    return (0, aiService_1.buildSystemPrompt)('an expert legal case preparation specialist with 20 years of UK and international legal experience', `You prepare comprehensive, structured legal case documents for individuals pursuing formal proceedings.
...
function buildCaseUserPrompt(input) {
    ...
    const evidenceInfo = input.evidenceCount > 0
        ? `${input.evidenceCount} evidence item(s) submitted.\nEvidence Summary: ${input.evidenceSummary ?? 'Not yet analysed'}`
        : 'No evidence submitted yet';
    const additionalContext = input.intakeData && Object.keys(input.intakeData).length > 0
...
async function generateCaseDocument(input) {
    const systemPrompt = buildCaseSystemPrompt();
    const userMessage = buildCaseUserPrompt(input);
    const messages = [{ role: 'user', content: userMessage }];
    try {
        logger_1.logger.info({ caseId: input.caseId }, 'Starting case document generation');
        ...
        logger_1.logger.error({ err, caseId: input.caseId }, 'Case generation failed - using fallback');
        return {
...
function parseAndValidateCaseResponse(content, input) {
    let raw;
    try {
    ...
    const legalArguments = Array.isArray(raw.legalArguments)
    ...
    return {
        ...
        executiveSummary: typeof raw.executiveSummary === 'string' && raw.executiveSummary.length > 20
        ...
        legalArguments,
        recommendedActions,
        strengthScore: typeof raw.strengthScore === 'number'
            ? Math.min(100, Math.max(0, Math.round(raw.strengthScore)))
            : 50,
        riskAssessment: typeof raw.riskAssessment === 'string' && raw.riskAssessment.length > 20
            ? raw.riskAssessment
            : 'Risk assessment could not be completed automatically. Legal advice is recommended.',
    };
}
function validateLegalArgument(arg) {
    const a = arg;
    return {
        ...
        supportingEvidence: Array.isArray(a.supportingEvidence)
            ...
            : [],
        strength: a.strength === 'strong' || a.strength === 'moderate' || a.strength === 'weak'
            ? a.strength
            : 'moderate',
    };
}
function validateRecommendedAction(act) {
    const a = act;
    return {
        priority: a.priority === 'immediate' || a.priority === 'short-term' || a.priority === 'long-term'
            ? a.priority
            : 'short-term',
...
function generateFallbackCase(input) {
    return {
...
NOTE: This case document was generated using the fallback system. A full AI-generated document will be available once the AI service is restored. Legal advice is strongly recommended.`,
        ...
        strengthScore: 50,
        riskAssessment: 'Automated risk assessment unavailable. Please consult a qualified legal advisor to assess the strength and risks of this case.',
    };
}
function generateFallbackArguments(input) {
    return [
        {
...
function generateFallbackActions(input) {
    const actions = [
        {
            priority: 'immediate',
...
//# sourceMappingURL=caseGenerationEngine.js.map